...
/**
 * Generates a collision-resistant, routable ID for a transaction.
...
/**
 * Checks that a value looks like a well-formed transaction object.
 * Used to filter out corrupted/partial entries from localStorage.
 */
 ...
 * Falls back to an empty array if the key is missing, unreadable,
 * not valid JSON, not an array, or contains malformed entries.
 */
 ...
 * useTransactions
 *
 * Single source of truth for all transaction data. Encapsulates
 * localStorage persistence so pages never read/write storage directly.
 */
export function useTransactions() {
  const [transactions, setTransactions] = useState(readStoredTransactions);

  // Persist to localStorage any time the transaction list changes.
  useEffect(() => {
    try {
      localStorage.setItem(STORAGE_KEY, JSON.stringify(transactions));
    } catch {
      // Storage unavailable (e.g. quota exceeded, private browsing) —
      // fail silently, in-memory state still works for this session.
    }
  }, [transactions]);
